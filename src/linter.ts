/**
 * linter — vendored markdown / frontmatter linter for compasscrew vaults.
 *
 ...
 * matches the compasscrew frontmatter conventions out of the box — no third
 ...
 *
 * Commands:
 *   - `compasscrew-lint-current-file` — applies enabled rules to active note.
 *   - `compasscrew-lint-vault`        — applies enabled rules to every md file
...
export function registerLinter(
  ...
  plugin.addCommand({
    id: "compasscrew-lint-current-file",
    name: "CompassCrew: lint current note (frontmatter + markdown style)",
    checkCallback: (checking: boolean) => {
      ...
      void (async () => {
        ...
        const s = getSettings();
        if (!s.lintEnabled) { new Notice("CompassCrew linter disabled in settings."); return; }
  ...
  plugin.addCommand({
    id: "compasscrew-lint-vault",
    name: "CompassCrew: lint entire vault (frontmatter + markdown style)",
    callback: () => {
      const s = getSettings();
      if (!s.lintEnabled) { new Notice("CompassCrew linter disabled in settings."); return; }
