...
class QuickThreadModal extends Modal {
  ...
  onOpen() {
    ...
    this.resultsEl = contentEl.createDiv({ cls: "compasscrew-thread-results" });
  ...
  refresh() {
    ...
    this.filtered.forEach((f) => {
      const row = this.resultsEl.createEl("div", { cls: "compasscrew-thread-row" });
      row.style.padding = "4px 8px";
      row.style.cursor = "pointer";
      row.style.borderBottom = "1px solid var(--background-modifier-border)";
      row.createEl("strong", { text: f.basename });
      row.createEl("span", { text: ` — ${f.parent?.path || ""}`, cls: "compasscrew-thread-path" });
      (row.querySelector(".compasscrew-thread-path") as HTMLElement).style.opacity = "0.6";
      (row.querySelector(".compasscrew-thread-path") as HTMLElement).style.fontSize = "0.85em";
...
class ThreadSuggestionsModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl("p", {
      text: `CompassCrew scanned your vault and found ${this.suggestions.length} relationships that could be threaded. Review and accept the ones that look right.`,
    });
    const accepted: ThreadSuggestion[] = [];
    this.suggestions.slice(0, 50).forEach((s, i) => {
      const row = contentEl.createDiv({ cls: "compasscrew-suggestion-row" });
...
function registerInlineWidget(plugin: Plugin) {
  plugin.registerMarkdownPostProcessor((el, ctx) => {
    ...
    if (el.querySelector(".compasscrew-thread-widget")) return; // dedupe
    ...
    const widget = el.createDiv({ cls: "compasscrew-thread-widget" });
    ...
    addBtn.onclick = () => {
      (plugin.app as any).commands.executeCommandById("compasscrew:compasscrew-quick-thread");
    };
  });
}
...
export function registerBreadcrumbsOnboarding(plugin: Plugin) {
  ...
  plugin.addCommand({
    id: "compasscrew-quick-thread",
    name: "CompassCrew: quick-thread (pick a note, pick a direction)",
  ...
  plugin.addCommand({
    id: "compasscrew-suggest-threads",
    name: "CompassCrew: suggest threads (auto-detect from folder + link structure)",
    callback: async () => {
      const suggestions = [
  ...
  plugin.addCommand({
    id: "compasscrew-breadcrumbs-tutorial",
    name: "CompassCrew: breadcrumbs tutorial (interactive)",
    callback: async () => {
      ...
      if (!fs.existsSync(target)) {
        const tut = [
          "---",
          "type: tutorial",
          "tags: [compasscrew, breadcrumbs, threading]",
          "---",
          "",
          ...
          "3. **CompassCrew: suggest threads** — scans your vault and proposes threads from folder structure + mutual links.",
          "",
          "## The mental model",
          "",
          ...
          "## Visual feedback",
          "",
          "CompassCrew's inline thread widget renders at the top of every threaded note:",
          "",
