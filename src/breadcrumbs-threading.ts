...
export function registerBreadcrumbsThreading(
  ...
  plugin.addCommand({
    id: "compasscrew-thread-add",
    name: "CompassCrew: add Breadcrumbs thread link (bearing → up/next/same)",
    callback: async () => {
  ...
  (plugin as any).compasscrewAddBreadcrumb = async (file: TFile, bearing: Bearing, dest: string) => {
    if (!isBearing(bearing)) return;
    await addBreadcrumbThread(plugin.app, file, bearing, dest, getEmitAliases());
  };

  (plugin as any).compasscrewReadBearings = (file: TFile) => readBearingFrontmatter(plugin.app, file);

  plugin.addCommand({
    id: "compasscrew-thread-open",
    name: "CompassCrew: open Breadcrumbs trail view",
    callback: () => {
      ...
      new Notice("Breadcrumbs plugin command not found. Install/enable breadcrumbs (run CompassCrew: doctor).", 8000);
    },
  });
}
