...
import { RECKON_SIGNUP_URL as COMPASSCREW_SIGNUP_URL, TOOL, TierGateError, callReckonTool, resolveToken } from "./reckon-contract";
 ...
 *           │
 *           │  (3b) "CompassCrew: push prompt back" → POST reckon_prompt verb=update
 *           ▼
...
/**
 * Resolve the effective bearer token through the ONE resolver in `reckon-contract.ts`, which
 * also honours the pre-rebrand token filenames and falls back to the read-only demo bearer.
 */
function readToken(app: App, s: SystemPromptSettings): string {
  return resolveToken(fs, path.join, vaultRoot(app), s.tokenPath).token;
}
...
export function registerSystemPrompt(plugin: Plugin, getSettings: () => SystemPromptSettings) {
    ...
    callback: async () => {
      ...
      try {
        await callReckonTool({
          mcpUrl: s.mcpUrl, token, tool: TOOL.PROMPT,
          args: { verb: "update", prompt_name: promptFile, new_content: proposed },
        });
        new Notice("Prompt update submitted — the server commits it through the git serializer.");
      } catch (e) {
        if (e instanceof TierGateError) {
          new Notice(`Sign-in required to push prompts.\nUpgrade: ${e.upgradeUrl || COMPASSCREW_SIGNUP_URL}`, 10000);
          return;
        }
        new Notice(`Push failed: ${(e as Error).message}`, 8000);
      }
    },
  });
  ...
  plugin.addCommand({
    ...
    callback: async () => {
      ...
      try {
        const data = await callReckonTool<{ ok?: boolean; error?: string }>({
          mcpUrl: s.mcpUrl, token, tool: TOOL.PROMPT,
          args: { verb: "update", prompt_name: promptName, new_content: newContent },
        });
        if (data?.ok) {
          new Notice(
        ...
        if (e instanceof TierGateError) {
          new Notice(
            ...
            `Upgrade at: ${e.upgradeUrl || COMPASSCREW_SIGNUP_URL}`, 12000);
          return;
        }
        new Notice(`Push failed: ${(e as Error).message}`, 8000);
      }
