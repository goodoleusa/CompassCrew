...
export const VIEW_TYPE_RECKON_CHARTERS = "reckon-charters";
 ...
 *
 * Styles: see styles.css section 6 (.reckon-charter-pane, -card, -form).
 *
...
export class ReckonChartersView extends ItemView {
  ...
  getViewType() { return VIEW_TYPE_RECKON_CHARTERS; }
  getDisplayText() { return "Reckon Charters"; }
  ...
  async onOpen() {
    ...
    this.root.empty();
    this.root.addClass("reckon-charter-pane");
    this.render();
  }
  ...
  private async render() {
    ...
    // List
    const list = this.root.createDiv({ cls: "reckon-charter-list" });
    ...
    for (const c of charters) {
      const card = list.createDiv({ cls: "reckon-charter-card" });
  ...
  private async openEditForm(charter_id: string) {
    ...
    const form = this.root.createDiv({ cls: "reckon-charter-form" });
...
export function registerCharterDashboard(
  ...
  plugin.registerView(
    VIEW_TYPE_RECKON_CHARTERS,
    (leaf) => new ReckonChartersView(leaf, getMcpUrl, getTokenPath),
  );
  plugin.addCommand({
    id: "reckon-open-charters",
    name: "✶ Reckon: open Charters dashboard",
    callback: async () => {
      const { workspace } = plugin.app;
      let leaf = workspace.getLeavesOfType(VIEW_TYPE_RECKON_CHARTERS)[0];
      if (!leaf) {
        leaf = workspace.getRightLeaf(false)!;
        await leaf.setViewState({ type: VIEW_TYPE_RECKON_CHARTERS, active: true });
      }
