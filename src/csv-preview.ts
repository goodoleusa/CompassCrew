...
/**
 * Hand-rolled CSV reader and Markdown table renderer for the `compasscrew-csv`
  ...
  plugin.registerMarkdownCodeBlockProcessor(
    ...
    async (source: string, el: HTMLElement, _ctx: MarkdownPostProcessorContext) => {
      try {
        ...
        if (!pathLine) {
          el.createEl("pre", { text: "compasscrew-csv: missing `path:` directive" });
          return;
        }
        const abs = resolveCsvPath(plugin.app, pathLine);
        if (!fs.existsSync(abs)) {
          el.createEl("pre", { text: `compasscrew-csv: file not found: ${abs}` });
          return;
        }
        const preview = readCsvPreview(abs, maxRows);
        const tbl = el.createEl("table", { cls: "compasscrew-csv-preview" });
        ...
        if (preview.truncated) {
          el.createEl("div", {
            ...
            cls: "compasscrew-csv-truncated",
          });
        }
      } catch (e) {
        el.createEl("pre", { text: "compasscrew-csv error: " + (e as Error).message });
      }
    },
  );

  plugin.addCommand({
    id: "compasscrew-csv-preview-clipboard",
    name: "CompassCrew: preview CSV from clipboard path",
    callback: async () => {
