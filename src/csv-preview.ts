...
/**
 * Hand-rolled CSV reader and Markdown table renderer for the `reckon-csv`
  ...
  plugin.registerMarkdownCodeBlockProcessor(
    ...
    async (source: string, el: HTMLElement, _ctx: MarkdownPostProcessorContext) => {
      try {
        ...
        if (!pathLine) {
          el.createEl("pre", { text: "reckon-csv: missing `path:` directive" });
          return;
        }
        const abs = resolveCsvPath(plugin.app, pathLine);
        if (!fs.existsSync(abs)) {
          el.createEl("pre", { text: `reckon-csv: file not found: ${abs}` });
          return;
        }
        const preview = readCsvPreview(abs, maxRows);
        const tbl = el.createEl("table", { cls: "reckon-csv-preview" });
        ...
        if (preview.truncated) {
          el.createEl("div", {
            ...
            cls: "reckon-csv-truncated",
          });
        }
      } catch (e) {
        el.createEl("pre", { text: "reckon-csv error: " + (e as Error).message });
      }
    },
  );

  plugin.addCommand({
    id: "reckon-csv-preview-clipboard",
    name: "Reckon: preview CSV from clipboard path",
    callback: async () => {
